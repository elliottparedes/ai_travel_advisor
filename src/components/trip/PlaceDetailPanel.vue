...
import type { DiscoveryCard, PlaceDetails, Review } from "../../types/trips";
...
async function loadReviews(fsqId: string) {
...
async function submitReview() {
  if (!auth.sessionToken || !props.card.fsqId || !reviewText.value.trim()) return;
  isSavingReview.value = true;
  try {
    await reviewsApi.writeReview({
      token: auth.sessionToken,
      fsqId: props.card.fsqId,
      placeTitle: props.card.title,
      city: props.city ?? "",
      country: props.country ?? "",
      placeType: props.card.type,
      vibe: reviewVibe.value,
      text: reviewText.value.trim(),
    });
    await loadReviews(props.card.fsqId);
...
async function voteHelpful(reviewId: string) {
  ...
  if (props.card.fsqId) await loadReviews(props.card.fsqId);
}
...
watch(() => props.card, async (card) => {
  ...
  const [more] = await Promise.all([
    tripsApi.fetchPlacePhotos(card.title).catch(() => [] as string[]),
    card.fsqId ? loadReviews(card.fsqId) : Promise.resolve(),
  ]);
...
<template>
  ...
  <div class="detail-panel" :class="{ 'detail-panel--closing': isClosing }">
    ...
    <div class="panel-body">
      ...
      <!-- Reviews section -->
      <div v-if="card.fsqId" class="reviews-section">
